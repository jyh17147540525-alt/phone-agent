...
enum class PluginCapability(
    ...
    // ── 用户文件（v3.1 新增）────────────────────────────────
    //
    // ⚠️ 与 [STORAGE] 是**两件事**，不要合并：
    //
    //    STORAGE   = 插件自己的独立空间。用户看不到，也碰不到别的插件的数据。
    //    FILE_*    = 用户在系统文件选择器里授权给本体的**真实目录**。
    //
    //    合并成一个能力会让"保存我的配置"和"读你的文档"看起来一样危险 ——
    //    而前者是零风险的。能力粒度太粗，用户就只能在"全给"和"全不给"
    //    之间选，那等于没有粒度。
    //
    // ★ 这三个能力**不是**逐项放行就完了：即使插件拿到了 file.write，
    ...
    FILE_WRITE(
        "file.write",
        "在你指定的目录里新建文件，或修改已有文件（修改会覆盖原内容，每次都会先问你）",
        RiskLevel.HIGH,
    ),
    FILE_DELETE(
        "file.delete",
        "删除你指定目录里的文件（每次都会先问你，删除后无法撤销）",
        RiskLevel.CRITICAL,
    ),
