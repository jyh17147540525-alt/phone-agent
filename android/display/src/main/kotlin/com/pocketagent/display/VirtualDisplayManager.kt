 ...
 *
 * ═══════════════════════════════════════════════════════════════
 *  ⚠️⚠️ 2026-09-25 状态修正 —— 先读这段，本文件**尚未反映最新结论**
 * ═══════════════════════════════════════════════════════════════
 *
 * 本文件写于**旧的 overlay 路线**时期（`settings put global overlay_display_devices`
 * + `screencap -d`）。后续真机实验已把其中**三条判断推翻**：
 *
 *  ① ❌ 原文：「**无障碍通道无法操作虚拟屏**（只能读，不能注入输入）」
 *     ✅ 实测：`input -d <displayId> tap` **真实生效**（帧 md5 变化 + 页面导航）。
 *        输入走的是**窗口自己的 input channel**，不是 display 的 touch 属性
 *        —— 所以 `dumpsys display` 报 `touch NONE` 也不影响注入。
 *
 *  ② ❌ 原文：「**副屏窗口在主屏上可见**」（见文件末尾 [DisplayLimitations]）
 *     ✅ 实测：VDM 路线**主屏零悬浮层**。「叠加视图 #1」是 **overlay 路线**特有的，
 *        而 overlay 路线已被否决。
 *
 *  ③ ❌ 原文：「**截取虚拟屏只能靠 `screencap -d`**」
 *     ✅ 实测：VDM 路线自带 `ImageReader` 消费面，**直接读像素**，无需截屏命令。
 *
 * **真正仍然成立的硬约束**（这些才是要花力气的地方）：
 *  · 跨应用跳转会切走主屏（`flags` 组合可缓解，但代价见交接文档）；
 *  · ❗ **隐私红线**：像素帧里会出现用户真实私人内容 ⇒ 读屏前必须先装 `PrivacyFilter`；
 *  · 部分 App 无法在副屏正常渲染（需降级兜底）；
 *  · 副屏不会随进程退出自动清理（**必须做孤儿屏检测**）。
 *
 * ★ **实现本接口之前，务必先读**：`docs/无感虚拟屏方案存档与交接-v1.0.md`
 *    —— 里面有可用的 flags 组合（`85003`）、三层约束推导、以及仍未闭环的问题。
 *
 * ═══════════════════════════════════════════════════════════════
 *  （以下为原始设计说明，部分已被上方的修正取代）
 * ═══════════════════════════════════════════════════════════════
 *  ⚠️ 先读这段，否则会写出跑不通的代码
 * ═══════════════════════════════════════════════════════════════
 *
...
object DisplayLimitations {
    val items: List<Limitation> = listOf(
        Limitation(
            // ⚠️ 2026-09-25 修正：原文写「副屏窗口在主屏上可见」——那是 **overlay 路线**的行为。
            //    VDM 路线实测**主屏零悬浮层**，本条已不成立，替换为真正会咬人的那条。
            title = "任务可能影响主屏",
            detail = "部分场景下（如从副屏触发跨应用跳转），主屏前台可能被切换到其他应用。" +
                "这是 Android 的显示组机制导致的，不是本应用能完全消除的。" +
                "我们会尽量把影响降到最低，并在发生时给出提示。",
        ),
        Limitation(
