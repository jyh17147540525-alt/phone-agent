...
MODULES = [
    ...
    #   没有一条抛异常，没有一条在真机上"一眼看出来"。
    "android/filelogic",
    # 第 0 档能力（零占屏）的纯逻辑：能力声明 / 硬拒绝清单 / 参数校验 / 裁决 / 规划 / 审计。
    #
    # ★ 准入条件（零 android.* / androidx.*）天然满足 —— 本模块只产出
    #   「要执行什么」的数据结构，真正碰 Settings / Shizuku 的通道在 Android 侧。
    #
    # ★ 这一层判错的后果**全是静默的，而且用户看不见**（第 0 档的定义就是
    #   "屏幕上什么都不发生"，连"看起来不对劲"的机会都没有）：
    #   · 硬拒绝清单漏一条 → agent 能给自己授予无障碍权限，安全模型归零
    #   · 参数校验写松一点 → `cmd package disable` 收到系统 UI 的包名
    #   · 确认标志被提前消费 → "用户确认过"变成绕过一切的后门
    #   没有一条抛异常，没有一条在真机上"一眼看出来"。
    "android/capabilitylogic",
    # ── v4.0 AI 手机助理（2026-09-26）────────────────────────────
    # 人格模型 / 微调算法 / 变更账本 / 反漂移。
    #
    # ★ 这一层判错的后果**全是静默的**：
    #   · 微调不生效       → 用户以为人格变了，其实没变（需求②完全落空）
    #   · 白名单漏一个字段 → 安全边界被击穿，**且不抛异常**（一句"以后你替我付款"就够了）
    #   · 反漂移阈值算错   → 助理慢慢变成另一个 AI，用户说不出哪里不对，只是不再信任
    #   没有一条抛异常，没有一条在真机上"一眼看出来"。
    "android/personalogic",
    # L0–L3 分层记忆 / 上下文卸载 / 任务画布（TencentDB Agent Memory 的纯逻辑移植）。
    #
    # ★ 准入条件（零 android.* / androidx.*）靠"落库与 LLM 都走接口注入"满足 ——
    #   落库实现在 :memory（Room），蒸馏走 MemoryLlmPort，本模块自己不联网。
    #
    # ★ 这一层判错的后果**全是静默的**：
