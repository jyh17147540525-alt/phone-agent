...
- ✅ 工程骨架（35 个 Gradle 模块，128 个 Kotlin 源文件，约 3.7 万行）
...
- ✅ **667 个离线单元测试全绿**（含 44 条真 socket 集成测试）
...
- ❌ **agent 循环所需的三个横切组件** —— `AgentBudget` / `TaskCheckpoint` / `PerceptionLadder`
...
网关本身已经能用了（有 667 条测试覆盖），但还没有任何界面把它接到用户手上 ——
...
共 35 个模块，详见 [`android/README.md`](android/README.md) 与 [方案文档](docs/)。
...
是**编译期**解析的。35 个模块里任何一个写错别名，配置阶段就失败，而且报错指向
