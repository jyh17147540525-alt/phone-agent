...
> **v4.0 方向变更（2026-09-26，设计稿待评审）**：项目正从「agent 工具」转向
> 「**AI 手机助理**」（人格 + 记忆 + 实时语音）。现有能力层**全部保留**，降为助理的执行器。
> 设计稿见 [`docs/AI手机助理架构-v1.0.md`](docs/AI手机助理架构-v1.0.md)。
> ⚠️ 该方向**目前只有设计稿与模块骨架** —— 6 个新模块已登记，但**尚无源码**。

已完成：
- ✅ 完整项目方案（v1.0 / v2.0 / v3.0）与 M0 技术验证手册（18 个实验）
- ✅ 工程骨架（44 个 Gradle 模块，201 个 Kotlin 源文件，约 6.1 万行）
...
- ✅ **网关与 dsh 的配置接线**（1.4.6c）—— `DshConfigPatch` 渲染 dsh 的
  `settings.yaml` 段与凭据条目（**真 Key 永不落盘，只给本地一次性 token**），
  `DshRoutingModeProvider` 提供真实路由模式，`DshConfigSink` 负责投递。
  **已真机端到端验证**：dsh 经此配置把请求发到本地网关并渲染出回答
- ✅ **Agent 循环的纯逻辑层**（S2/S3，`agentlogic`）—— 四维预算熔断（轮次/时长/能耗/流量）、
  任务状态机 + 卡死检测、五档感知阶梯、检查点与中断恢复、定位消歧
- ✅ **`PrivacyFilter` 隐私关卡**（S3 第 4/5 个横切组件）—— **所有上传路径的唯一出口**：
  敏感页面一票否决、插件截图一律拒绝、系统 UI 区域未知时保守丢弃、
  输入框/消息区遮蔽指令、绕过检测
- ✅ **文件能力沙箱**（`:filelogic`）—— 让 agent 读写用户指定范围内的文件、确认后删除。
  范围模型（SAF 授权 + 只许加严的位置黑名单）、路径归一化与穿越防护（**段级包含判定**、
  不做 NFKC / 大小写折叠 / 百分号解码 / 符号链接解析）、操作裁决（**覆盖与删除必须确认，
  且确认不能绕过前置检查**）、防伪造审计日志、文本 / Markdown / CSV / JSON 组装
  （含 CSV 公式注入中和）。**判定层纯 Kotlin，可离线完整测试**；
  Android 侧通道实现待下一轮。详见 [`docs/文件能力沙箱设计-v1.0.md`](docs/文件能力沙箱设计-v1.0.md)
- ✅ **1339 个离线单元测试全绿**（含 44 条真 socket 集成测试）+ **6 处变异验证**
  （证明关键判定确实被测试守住，而不只是"全绿"）
- ✅ **发行就绪检查器**（`check_release_readiness.py`，10 组规则 R1–R10 + 20 项自测）
...
- ❌ **`PowerGovernor`** —— 五横切组件里最后一个（前四个已实现）。
  它需要全局视角（App + Node + 网络），单层实现不了
- ❌ **网关接入 agent 循环**（1.4.8）—— 网关与 dsh 的配置接线（1.4.6c）已完成并真机验证
...
网关本身已经能用了（有 925 条测试覆盖，且已真机验证 dsh 能经它取到回答），
但还没有任何界面把它接到用户手上 ——
...
共 44 个模块，详见 [`android/README.md`](android/README.md) 与 [方案文档](docs/)。
...
> **`gen_module_build_files.py --check` 目前是零容忍的**（43 个模块必须全部一致）。
...
是**编译期**解析的。44 个模块里任何一个写错别名，配置阶段就失败，而且报错指向
